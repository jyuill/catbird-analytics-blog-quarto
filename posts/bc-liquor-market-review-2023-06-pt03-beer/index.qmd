...
We can see the overall decline in litre sales of BC-produced beer is driven by declines in 'Commercial' beer, which Regional breweries have actually grown, and Micro Brews have had some slow growth.
...
```{r}
trend_yr_subcat %>% ggplot(aes(x=year, y=pc_chg_litres))+geom_col()+
  facet_grid(.~subcat)+
  scale_y_continuous(labels=percent_format())+
...
-   Commercial beer had negative year-over-year growth every year - with the low being almost -105 in 2019.
-   
