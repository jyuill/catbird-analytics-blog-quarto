...
```{r}
# ch_title <- "Beer: Net Sales $ Trends - All Categories"
...
Beer sales have been pretty stable over the last few years, with peak in 2017 for litre sales at just under 300M and slight downward trend since then. Net \$ sales peaked around the same time and also trended down, but increased in 2022 with inflation.

```{r, sales dual-axis}
# try dual axis
ch_title <- "BC Beer: Net Sales $ Trends + Litre Sales"
trend_yr %>% ggplot(aes(x=as.factor(year), y=netsales, group=1))+
  geom_line(color=bar_col, size=2)+
  geom_point(aes(y=netsales), color=bar_col, size=3)+
...
trend_yr_qtr %>% ggplot(aes(x=end_qtr_dt, y=litres, group=1))+
  ...
  scale_y_continuous(labels=comma_format(scale=1e-6,suffix="M"), 
                     expand=expansion(mult=c(0,0.1)), limits=c(0,max(trend_yr_qtr$litres)))+
  labs(title=ch_title, x="")
```

### Sales by Category

BC LDB puts beer into 3 categories for purposes of the Liquor Market Report:
...
ch_title <- "BC Beer Litre Sales by Category"
...
ch_title <- "BC Beer Litre Sales by Category"
plot_right <- trend_yr_cat %>% ggplot(aes(x=as.factor(year), y=litres, fill=category))+
