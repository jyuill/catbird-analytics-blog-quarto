...
-   in % terms, litre market share for BC remained steady just over 75%, while other provinces totally took their market share out of imports.

```{r}
# side-by-side charts
#| fig-cap: Beer sales by category
#| label: beer
#| layout-ncol: 2
...
```{r}
ch_title <- "BC Beer Litre Sales by Category"
plot <- trend_yr_cat %>% ggplot(aes(x=as.factor(year), y=litres, fill=category))+
  geom_col()+
  scale_y_continuous(labels=comma_format(scale=1e-6,suffix="M"), 
                     expand=expansion(add=c(0,0.1)))+
  #scale_fill_manual(values=type_color)+
